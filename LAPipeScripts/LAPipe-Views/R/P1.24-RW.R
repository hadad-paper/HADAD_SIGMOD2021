...
V1<-as.matrix(read.table(file=paste(args[2],"",sep=""), header = FALSE, sep = ","))
V7<-as.matrix(read.table(file=paste(args[3],"",sep=""), header = FALSE, sep = ","))
...
write.table(paste("P1.24RW:",args[1],sep=""), 
            path, 
            append = TRUE, 
            row.names = FALSE,
