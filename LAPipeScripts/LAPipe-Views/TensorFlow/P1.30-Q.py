...
def Query_Call(Mdata,Ndata,Rdata):
    G = tf.Graph()
    with G.as_default():
        ...
        pipe = tf.multiply(tf.matmul(N,M),tf.matmul(tf.matmul(M,N),tf.transpose(R)))
        init = tf.global_variables_initializer()
        with tf.Session() as sess:
            sess.run(init)
            res = sess.run(pipe, feed_dict={M: Mdata,N:Ndata, R:Rdata})
            logging.info('QueryResult: {}'.format(pd.DataFrame(res).at[0,1]))

    return res


Query = 'Query_Call(M,N,R)'
runTimes.ix[:,'Query'] = "P1.30(Q"+str(sys.argv[1])+")"
