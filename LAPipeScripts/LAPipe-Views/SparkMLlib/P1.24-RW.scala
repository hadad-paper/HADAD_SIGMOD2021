...
def test07(V1D: DenseMatrix, V7D: DenseMatrix, DD: DenseMatrix) : Double= {
    return  bAlg.trace(as_breeze(V1D.multiply(V7D))) + bAlg.trace(as_breeze(DD))
}
...
for (ix <- 0 to 5) {
    ...
    val res =test07(V1D,V7D,DD)
    println(res)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P1.24(RW"+scriptArgs(0)+"): ");
