...
def rw(V: DenseMatrix, D: DenseMatrix): DenseMatrix= {
    return  V.multiply(D).transpose
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val V1 =  readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
V1.blocks.count
val C = readDMM(scriptArgs(1), sc).toBlockMatrix(100,100)
C.blocks.count

val V1D = C.toLocalMatrix.asInstanceOf[DenseMatrix]
val CD = D.toLocalMatrix.asInstanceOf[DenseMatrix]
...
File(outputPath).appendAll("P2.6(RW"+scriptArgs(0)+"): ");
