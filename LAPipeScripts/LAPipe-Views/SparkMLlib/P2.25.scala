...
def query(X: BlockMatrix, U:BlockMatrix,V:BlockMatrix):BlockMatrix = {
    return U.multiply(V.transpose,500).subtract(X).multiply(V,500)
}

def rw(X: BlockMatrix, V:BlockMatrix,V1:BlockMatrix):BlockMatrix = {
    return V.multiply(V1,500).subtract(X.multiply(V1,500))
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val X = readSMM(scriptArgs(0), sc).toBlockMatrix(100,100)
X.blocks.count
val U = readDMM(scriptArgs(1), sc).toBlockMatrix(100,100)
U.blocks.count

val V = readDMM(scriptArgs(2), sc).toBlockMatrix(100,100)
V.blocks.count
val view = U.multiply(V.transpose,500)
view.blocks.count
...
File(outputPath).appendAll("P2.25(RW"+scriptArgs(0)+"): ");
