...
def query(M: BlockMatrix, N:BlockMatrix):BlockMatrix = {
     val mat = N.multiply(M,500)
    return M.multiply(mat,500).transpose
}
...
File(outputPath).appendAll("P2.20(Q"+scriptArgs(0)+"): ");
