...
def query(V6D: DenseMatrix):linalg.Vector = {
    val add =as_breeze_D(V6D)
    return  from_breeze(bAlg.sum((add)(::,*)).t)
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val V6 = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
V6.blocks.count
val V6D = V6.toLocalMatrix.asInstanceOf[DenseMatrix]
//Query
...
File(outputPath).appendAll("P2.18(RW"+scriptArgs(0)+"): ");
