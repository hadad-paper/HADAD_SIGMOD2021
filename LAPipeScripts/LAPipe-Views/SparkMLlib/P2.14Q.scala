...
def query(A: BlockMatrix, B:BlockMatrix):BlockMatrix = {
    return A.multiply(B,500).multiply(A,500).multiply(B,500)
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val M = readSMM(scriptArgs(0), sc).toBlockMatrix(100,100)
M.blocks.count
...
File(outputPath).appendAll("P2.14(Q"+scriptArgs(0)+"): ");
