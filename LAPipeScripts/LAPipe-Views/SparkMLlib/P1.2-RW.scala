...
def query(V: DenseMatrix): linalg.DenseMatrix = {
    return  from_breeze(as_breeze(V)).asInstanceOf[DenseMatrix].transpose
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val V6 = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
V6.blocks.count
val VD = V6.toLocalMatrix.asInstanceOf[DenseMatrix]
