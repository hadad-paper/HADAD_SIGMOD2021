...
def timeOp(string,cleanup=None):
    ...
    logname = '/mnt/TensorFlow/src1/Dense/log/P1.25_S-Q-RWLogFile{}.log'.format(time_stamp)
...
def RW_Call(Mdata,Ndata):
    ...
    with G.as_default():
        ...
        pipe = tf.multiply(M,tf.divide(tf.transpose(N),tf.matmul(M,tf.matmul(N,tf.transpose(N)))))
...
RW = 'RW_Call(M,N)'
runTimes.ix[:,'Query'] = "P1.25(RW"+str(sys.argv[1])+")"
