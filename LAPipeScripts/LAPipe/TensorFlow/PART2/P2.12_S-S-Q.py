...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P2.12-S-S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Mdata,Ndata):
    ...
    with G.as_default():
        ...
        pipe = tf.reduce_sum(tf.reduce_sum(tf.matmul(tf.transpose(N),tf.transpose(M)),1))
...
runTimes.ix[:,'Query'] = "P2.12(Q"+str(sys.argv[1])+")"
