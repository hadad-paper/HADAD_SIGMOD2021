...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P2.6_S-S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Cdata,Ddata):
    ...
    with G.as_default():
        ...
        pipe = tf.matmul(tf.transpose(C),tf.transpose(tf.linalg.inv(D)))
...
runTimes.ix[:,'Query'] = "P2.6(Q"+str(sys.argv[1])+")"
