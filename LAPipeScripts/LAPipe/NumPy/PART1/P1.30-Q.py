...
def Query_Call(M,N,R):
    return np.multiply(np.matmul(N,M),np.matmul(np.matmul(N,M),np.transpose(R)))  
...
R = genfromtxt(str(sys.argv[3]), delimiter=',')
Query = 'Query_Call(M,N,R)'
runTimes.ix[:,'Query'] = "P1.30Q"
