...
def test17(C: DenseMatrix,D: DenseMatrix) : DenseMatrix= {
    return  from_breeze (bAlg.inv(as_breeze(D))).multiply(from_breeze(bAlg.inv(as_breeze(D)))).multiply(C)
}
...
File(outputPath).appendAll("P2.7(Q"+scriptArgs(0)+"): ");
