...
def test03(C: DenseMatrix, D: DenseMatrix): DenseMatrix= {
    return  from_breeze(bAlg.inv(as_breeze(C))).multiply(from_breeze(bAlg.inv(as_breeze(D))))
}
...
File(outputPath).appendAll("P1.3(Q"+scriptArgs(0)+"): ");
