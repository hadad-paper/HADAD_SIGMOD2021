...
def test05(A: DenseMatrix, B: DenseMatrix, V1: DenseVector) : DenseVector= {
    return  from_breeze (as_breeze (A.multiply(V1)) + as_breeze(B.multiply(V1)))
}
...
val V1 = as_dense(vectorize(Matrices.rand(scriptArgs(1).toInt, 1, new Random())))


val times = Array[Double](0,0,0,0,0,0)
for (ix <- 0 to 5) {
    val start = System.nanoTime()
    val res =test05(A,B,V1)
    println(res.apply(1))
