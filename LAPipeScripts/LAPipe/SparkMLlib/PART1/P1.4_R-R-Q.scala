...
def query(A: SparseMatrix, B:SparseMatrix, V1: DenseVector): DenseVector = {
    return  from_breeze(as_breeze(AS) +  as_breeze(BS)).asInstanceOf[SparseMatrix].multiply(V1)
}
...
B.blocks.count
val V1 = as_dense(vectorize(Matrices.rand(scriptArgs(2).toInt, 1, new Random())))
