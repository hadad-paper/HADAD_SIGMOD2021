...
def test36(C: DenseMatrix,D: DenseMatrix) : Double= {
    return  bAlg.det(as_breeze(C.multiply(D).multiply(C)))
}
...
File(outputPath).appendAll("P1.17(Q"+scriptArgs(0)+"): ");
