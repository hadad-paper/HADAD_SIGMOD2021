...
def query(A: DenseMatrix, B:DenseMatrix):linalg.Vector = {
    val add = as_breeze_D(A.transpose) + as_breeze_D(B.transpose)
    return  from_breeze(bAlg.sum((add)(*,::)))
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")

val A = as_dense(Matrices.rand(scriptArgs(0).toInt, scriptArgs(1).toInt, new Random()))
val B = as_dense(Matrices.rand(scriptArgs(0).toInt, scriptArgs(1).toInt, new Random()))
...
File(outputPath).appendAll("P1.11(Q"+scriptArgs(0)+"): ");
