...
def query(A: DenseMatrix): linalg.Matrix = {
    implicit val s1: Double = 2.0
    implicit val s2: Double = 4.0
    return  from_breeze(((as_breeze(A) +  as_breeze(A)):*(s1+s2)))
}
...
File(outputPath).appendAll("P1.8(RW"+scriptArgs(0)+"): ");
