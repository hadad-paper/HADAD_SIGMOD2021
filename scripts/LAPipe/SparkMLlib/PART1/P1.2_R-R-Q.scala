...
def query(A: BlockMatrix, B:BlockMatrix): linalg.Matrix = {
    val AS = A.transpose.toLocalMatrix.asInstanceOf[DenseMatrix].toSparse
    val BS = B.transpose.toLocalMatrix.asInstanceOf[DenseMatrix].toSparse
    return  from_breeze(as_breeze(AS) +  as_breeze(BS))
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val A = readSMM(scriptArgs(0), sc).toBlockMatrix(100,100)
A.blocks.count
val B = readSMM(scriptArgs(1), sc).toBlockMatrix(100,100)
B.blocks.count
...
File(outputPath).appendAll("P1.2(Q"+scriptArgs(0)+"): ");
