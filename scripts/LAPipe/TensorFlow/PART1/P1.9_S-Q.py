...
D = genfromtxt('/mnt/TensorFlow/input/'+str(sys.argv[1]), delimiter=',')
...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P1.9_S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Ddata):
    G = tf.Graph()
    with G.as_default():
        D = tf.placeholder(tf.float64, shape=Ddata.shape)
        pipe = tf.linalg.det(tf.transpose(D))
    ...
    return res
Query = 'Query_Call(D)'
runTimes.ix[:,'Query'] = "P1.9(Q"+str(sys.argv[1])+")"
