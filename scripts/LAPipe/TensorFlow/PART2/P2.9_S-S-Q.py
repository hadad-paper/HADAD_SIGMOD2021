...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P2.9_S-S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Cdata,Ddata):
    ...
    with G.as_default():
        ...
        pipe = tf.linalg.trace(tf.matmul(tf.transpose(C),tf.transpose(D))+D)
...
runTimes.ix[:,'Query'] = "P2.9(Q"+str(sys.argv[1])+")"
