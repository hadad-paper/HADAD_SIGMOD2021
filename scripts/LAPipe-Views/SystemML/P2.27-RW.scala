...
val dmlTextQuery =
  s"""
    ...
    |  V9 = read("${V9}", format="csv", header=FALSE, sep=",");
    |  V5 = read("${V5}", format="csv", header=FALSE, sep=",");
    | for (ix in 1:5) {
    |   start = time(1)
...
val resultsQ = resQ.getTuple[Matrix]("timesRW")._1.to2DDoubleArray
File(outputPath).appendAll("P2.27RW("+scriptArgs(0)+"):");
