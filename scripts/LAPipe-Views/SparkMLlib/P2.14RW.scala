...
def readSMM(matirxName: String, sc: SparkContext) : IndexedRowMatrix = {
      val M = sc.textFile("/local/ralotaib/estocada-LA/SingleNode-Synthetic/data/"+matirxName).zipWithIndex()
...
def query(M: BlockMatrix, N:BlockMatrix, V:BlockMatrix):BlockMatrix = {
    return M.multiply(V,500).multiply(N,500)
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val M = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
M.blocks.count
val N = readDMM(scriptArgs(1), sc).toBlockMatrix(100,100)
N.blocks.count
val V3 = readDMM(scriptArgs(3), sc).toBlockMatrix(100,100)
V3.blocks.count

val outputPath = "DENSE.out"
if (!Files.exists(Paths.get(outputPath))) {
    File(outputPath).writeAll("-DENSE-\n")
}
//Query
val times = Array[Double](0,0)
for (ix <- 0 to 1) {
    val start = System.nanoTime()
    val res =query(M,N,V3)
    println(res.blocks.count)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P2.14(RW"+scriptArgs(0)+"): ");
