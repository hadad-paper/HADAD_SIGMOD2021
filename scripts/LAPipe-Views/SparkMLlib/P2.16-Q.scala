...
def test07(CD: DenseMatrix, DD: DenseMatrix) : Double= {
    return  bAlg.trace(as_breeze(from_breeze(bAlg.inv(as_breeze(CD))).multiply(from_breeze(bAlg.inv(as_breeze(DD)))))) + bAlg.trace(as_breeze(DD))
}
...
for (ix <- 0 to 5) {
    ...
    val res =test07(CD,DD)
    println(res)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P2.16(Q"+scriptArgs(0)+"): ");
