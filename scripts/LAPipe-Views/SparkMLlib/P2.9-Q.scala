...
def test16(CD: DenseMatrix,DD: DenseMatrix) : Double= {
    return  bAlg.trace(as_breeze(CD.transpose.multiply(DD.transpose)) + as_breeze(DD))
 }
...
for (ix <- 0 to 5) {
    ...
    val res =test16(CD,DD)
    println(res)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P2.9(Q"+scriptArgs(0)+"): ");
