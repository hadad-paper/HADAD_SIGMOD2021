...
def test07(VD: DenseMatrix) : Double= {
    return  bAlg.trace(as_breeze(VD))
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val V7 = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
V7.blocks.count
val VD = V7.toLocalMatrix.asInstanceOf[DenseMatrix]
...
File(outputPath).appendAll("P1.20(RW"+scriptArgs(0)+"): ");
