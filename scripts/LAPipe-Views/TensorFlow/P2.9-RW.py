...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P2.9LogFile{}.log'.format(time_stamp)
...
def RW_Call(Vdata,Ddata):
    G = tf.Graph()
    with G.as_default():
        V = tf.placeholder(tf.float64, shape=Vdata.shape)
        D = tf.placeholder(tf.float64, shape=Ddata.shape)
        pipe = tf.linalg.trace(V)+ tf.linalg.trace(D)
        init = tf.global_variables_initializer()
        with tf.Session() as sess:
            sess.run(init)
            res = sess.run(pipe, feed_dict={V:Vdata, D:Ddata})
...
RW = 'RW_Call(V12,D)'
runTimes.ix[:,'Query'] = "P2.9(RW"+str(sys.argv[1])+")"
