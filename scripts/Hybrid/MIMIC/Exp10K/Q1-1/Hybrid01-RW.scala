...
val rRelationData = spark.sql("""SELECT subject_id, row_id, callout_outcome_cat 
                                 ...
                                 WHERE services.subject_id_s=callouts.subject_id AND 
                                       ...
                                       curr_careunit='CCU' and callout_outcome_cat=1 AND
                                       subject_id<=11000 AND
                                       row_id<= 35000 AND row_id!=6608""")
...
val outputPath = "Exp10K/Q1-1/results-RW.out"
...
val dmlTextQuery =
  s"""
    ...
    |  VV1 = read("Exp10K/Q1-1/VV1.csv", format="csv", header=FALSE, sep=",");
    |  N  = readMM("N.mtx");
