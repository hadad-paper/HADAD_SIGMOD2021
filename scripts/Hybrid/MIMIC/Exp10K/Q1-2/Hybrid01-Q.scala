...
val rRelationData = spark.sql("""SELECT subject_id, row_id, callout_outcome_cat 
                                 ...
                                 WHERE services.subject_id_s=callouts.subject_id AND 
                                       ...
                                       row_id!=6608 and 
                                       subject_id <=11000 and 
                                       row_id<= 35000""")

val rRelationDataDF = rRelationData.dropDuplicates()
//Construct N Matirx
constructSparseMatirx (11000,35000,rRelationDataDF,"N.mtx");

//Analysis
try{
val ml = new MLContext(spark)
val outputPath = "Exp10K/Q1-2/results-Q.out"
...
val dmlTextQuery =
  s"""
    ...
    |   print("TimeS"+((end2-start2)/1000))
    |   res1 = U%*%colSums(t(X%*%M))+NF
    |   while(FALSE){}
    |   print(as.scalar(res1[1,20]))
    |   print(as.scalar(res1[1,30]))
    |   print(as.scalar(res1[1,40]))
    |   print(as.scalar(res1[1,50]))
    |   print(as.scalar(res1[1,60]))
    |   print(as.scalar(res1[1,70]))
    |   print(as.scalar(res1[1,80])) 
    |   end = time(1)
    |   print("StartTime: "+end)
