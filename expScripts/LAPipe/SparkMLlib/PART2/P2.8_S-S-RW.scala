...
def test18(C: DenseMatrix,D: DenseMatrix) : Double= {
    return  bAlg.det(as_breeze(C.transpose.multiply(D)))
}
...
val C = as_dense(Matrices.rand(scriptArgs(0).toInt, scriptArgs(1).toInt, new Random()))
...
File(outputPath).appendAll("P2.8(RW"+scriptArgs(0)+"): ");
