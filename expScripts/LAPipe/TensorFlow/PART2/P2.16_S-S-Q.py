...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P2.20_S-S-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Cdata,Ddata):
    G = tf.Graph()
    with G.as_default():
        D = tf.placeholder(tf.float64, shape=Ddata.shape)
        C = tf.placeholder(tf.float64, shape=Cdata.shape)
        pipe = tf.linalg.trace(tf.matmul(tf.linalg.inv(C),tf.linalg.inv(D)))+ tf.linalg.trace(D)
...
runTimes.ix[:,'Query'] = "P2.20(Q"+str(sys.argv[1])+")"
