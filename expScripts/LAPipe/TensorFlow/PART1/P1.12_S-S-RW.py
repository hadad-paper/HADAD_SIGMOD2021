...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P1.12_R-S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def RW_Call(Mdata,Ndata):
    ...
    with G.as_default():
        ...
        pipe = tf.matmul(tf.reduce_sum(M,0),N)
        init = tf.global_variables_initializer()
        with tf.Session() as sess:
            sess.run(init)
            res = sess.run(pipe, feed_dict={M: Ndata,N: Mdata})

    return res

RW = 'RW_Call(M,N)'
runTimes.ix[:,'Query'] = "P1.12(RW"+str(sys.argv[1])+")"
