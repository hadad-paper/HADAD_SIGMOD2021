...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P1.4_S-S-Q-RW-LogFile{}.log'.format(time_stamp)
...
def Query_Call(Adata,Bdata,VData):
    G = tf.Graph()
    with G.as_default():
        ...
        pipe = tf.matmul((A+B),V)
        init = tf.global_variables_initializer()
        with tf.Session() as sess:
            sess.run(init)
            res = sess.run(pipe, feed_dict={A: Adata,B:Bdata,V:Vdata})
            logging.info('QueryResult: {}'.format(pd.DataFrame(res).at[0,1]))

    return res
Query = 'Query_Call(A,B,V)'
runTimes.ix[:,'Query'] = "P1.4(Q"+str(sys.argv[1])+")"
