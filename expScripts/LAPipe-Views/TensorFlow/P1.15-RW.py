...
M3 = genfromtxt(str(sys.argv[1]), delimiter=',')
...
def timeOp(string,cleanup=None):
    ...
    logname = 'log/P1.15LogFile{}.log'.format(time_stamp)
...
def Query_Call(Mdata,Ndata):
    ...
    with G.as_default():
        ...
        pipe = tf.matmul(tf.matmul(M,N),M)
...
RW = 'RW_Call(M,V3)'
runTimes.ix[:,'Query'] = "P1.15(RW"+str(sys.argv[1])+")"
