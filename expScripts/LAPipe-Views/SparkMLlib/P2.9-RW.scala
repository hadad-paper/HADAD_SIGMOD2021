...
def test16(V12D: DenseMatrix,DD: DenseMatrix) : Double= {
    return  bAlg.trace(as_breeze(V12D)) + bAlg.trace(as_breeze(DD))
 }
val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val D = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
D.blocks.count
val V12 = readDMM(scriptArgs(1), sc).toBlockMatrix(100,100)
V12.blocks.count

val DD = D.toLocalMatrix.asInstanceOf[DenseMatrix]
val V12D = V12.toLocalMatrix.asInstanceOf[DenseMatrix]


val times = Array[Double](0,0,0,0,0,0)
for (ix <- 0 to 5) {
    val start = System.nanoTime()
    val res =test16(V12D,DD)
    println(res)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P2.9(RW"+scriptArgs(0)+"): ");
