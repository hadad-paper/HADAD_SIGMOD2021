...
def query(C: DenseMatrix, D: DenseMatrix, y:DenseVector): DenseVector= {
    return  from_breeze(bAlg.inv(as_breeze(D.transpose.multiply(D)))).multiply(D.transpose.multiply(y))
}

def rw(V: DenseMatrix, D: DenseMatrix, y:DenseVector): DenseVector= {
    return  V.multiply(V.transpose.multiply(D.transpose.multiply(y)))
}
...
D.blocks.count
val y = vectorize(Matrices.rand(10000, 1, new Random()))


val V1D = V1.toLocalMatrix.asInstanceOf[DenseMatrix]
val DD = D.toLocalMatrix.asInstanceOf[DenseMatrix]

val timesRW = Array[Double](0,0,0,0,0,0)
for (ix <- 0 to 5) {
    val start = System.nanoTime()
    val res =rw(V1D,DD,y)
    println(res.apply(1))
    val stop = System.nanoTime()
    timesRW(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P2.21(RW"+scriptArgs(0)+"): ");
File(outputPath).appendAll(timesRW.mkString(",") + '\n')
System.exit(0)
