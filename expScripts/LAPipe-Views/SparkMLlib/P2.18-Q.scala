...
def query(AD: DenseMatrix, BD:DenseMatrix):linalg.Vector = {
    val add = as_breeze_D(AD.transpose) + as_breeze_D(BD.transpose)
...
File(outputPath).appendAll("P2.18(Q"+scriptArgs(0)+"): ");
