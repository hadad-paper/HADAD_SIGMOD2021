...
def test25(AD: DenseMatrix, BD: DenseMatrix) : Double= {
    return  bAlg.sum(as_breeze(AD)+as_breeze(BD))
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val A =  readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
A.blocks.count
val B =  readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
B.blocks.count
