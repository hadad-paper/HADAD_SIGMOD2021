...
def test07(V5D: DenseMatrix, CD: DenseMatrix) : DenseMatrix= {
    return  V5D.multiply(CD).multiply(CD)
}
...
val times = Array[Double](0,0,0,0,0,0,0)
for (ix <- 0 to 5) {
    val start = System.nanoTime()
    val res =test07(V5D,CD)
    println(res.apply(1,1))
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P1.29(RW"+scriptArgs(0)+"): ");
