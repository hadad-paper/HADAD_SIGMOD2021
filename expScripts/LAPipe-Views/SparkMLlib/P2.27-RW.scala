...
def rw(V1: DenseMatrix, V2: DenseMatrix): DenseMatrix= {
    return  V1.transpose.multiply(V2)
}

val scriptArgs = sc.getConf.get("spark.driver.args").split(",")
val V9 = readDMM(scriptArgs(0), sc).toBlockMatrix(100,100)
V9.blocks.count
val V5 =readDMM(scriptArgs(1), sc).toBlockMatrix(100,100)
V5.blocks.count

val V9D = V9.toLocalMatrix.asInstanceOf[DenseMatrix]
val V5D = V5.toLocalMatrix.asInstanceOf[DenseMatrix]
...
File(outputPath).appendAll("P2.27(RW"+scriptArgs(0)+"): ");
