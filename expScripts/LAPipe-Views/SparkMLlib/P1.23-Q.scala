...
def test15(CD: DenseMatrix,DD: DenseMatrix) : Double= {
    return  bAlg.det(bAlg.inv(as_breeze(CD.multiply(DD))) + as_breeze(DD))
}
...
for (ix <- 0 to 5) {
    ...
    val res =test15(CD,DD)
    println(res)
    val stop = System.nanoTime()
    times(ix) = (stop - start) / 1e9
}

File(outputPath).appendAll("P1.23(Q"+scriptArgs(0)+"): ");
