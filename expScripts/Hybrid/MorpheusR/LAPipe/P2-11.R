...
result <-double(20)
   for (t in range2){
       ...
       TNM = NormalMatrix(EntTable = list(S),
                   ...
                   Sparse = TRUE);
      TR = nrow(T)
      A= matrix(rexp(TR, rate=.1), nrow=TR,ncol=ncol(T))
      times_m <- double(4)
      times_f <- double(4)
      avg<-double(4)
      for (iter in 1:4) {
    ...
    write.table(rbind(result), file = "../../../figures/HybridResults/Morpheus/resultP2-10.csv", row.names =FALSE, col.names = FALSE,sep = ",",append = TRUE)
}
