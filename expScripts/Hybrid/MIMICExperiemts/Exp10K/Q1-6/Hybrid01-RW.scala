...
val outputPath = "Exp10K/Q1-6/results-RW.out"
...
val dmlTextQuery =
  s"""
    ...
    |  VV1  = read("Exp10K/Q1-6/VV1.csv", format="csv", header=FALSE, sep=",");
    |  N  = readMM("N.mtx");
    ...
    |   print(as.scalar(res[1000,1]))
    |   end = time(1)
    |   write(res, "RW.csv", format="csv", header=TRUE, sep=",", sparse=TRUE)
    |   print("StartTime: "+end)
