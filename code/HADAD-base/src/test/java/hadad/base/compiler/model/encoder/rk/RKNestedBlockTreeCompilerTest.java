...
public abstract class RKNestedBlockTreeCompilerTest extends AbstractQueryBlockTreeCompilerTest {
    ...
    private List<File> getTestsDirectories() {
        final String TESTS_DIR = "/compiler/rk-encoder";
        final File directory =
                new File(RKNestedBlockTreeCompilerTest.class.getResource(TESTS_DIR).toString().substring(5));
    ...
    private void test(final File testDirectory) {
        ...
        while (it.hasNext()) {
            ...
            System.out.println("Reformulation: " + reformulations.toString());
            if (it.hasNext()) {
