...
final class BlockListener {
    ...
    /**
     * Construct return template for NumPy script
     * 
     * @param patternVars
    ...
    private ReturnTemplate constructReturnTemplate(final List<Variable> patternVars) {
        ...
        terms.add(term);
        return new ReturnTemplate(NMModel.ID, terms);
    }
    ...
    private class BlockListenerAux extends NumPyBaseListener {
        ...
        public void enterNumPyScript(NumPyParser.NumPyScriptContext ctx) {
            //LOGGER.debug("Entering NumPy script block listener: " + ctx.getText());
            if (pattern == null) {
