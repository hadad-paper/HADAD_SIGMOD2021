...
final class BlockBackwardEncoderReturnTermVisitor implements
	...
	public List<List<Atom>> encode(
			final Map<Variable, PathExpression> definitions,
			final Variable parentBlockCreatedNode,
			final ReturnTemplate template, final String viewName) {
		return _encode(definitions, parentBlockCreatedNode, template,
				viewName);
	}
	...
	private List<List<Atom>> _encode(
			final Map<Variable, PathExpression> definitions,
			final Variable parentBlockCreatedNode,
			final ReturnTemplate template, final String viewName) {
	...
	public void visitPre(final ReturnConstructTerm term) {
		...
		builderAux.add(new Atom(Predicate.TAG.toString() + "_" + viewName, term
		...
		for (final Entry<String, ReturnLeafTerm> attr : term.getAttributes()
				.entrySet()) {
			final ImmutableList.Builder<Atom> builderAuxAttr = ImmutableList
					.builder();
			builderAuxAttr.addAll(builderAux.build());
			builderAuxAttr.add(new Atom(Predicate.HAS_ATTRIBUTE.toString()
					+ "_" + viewName, term.getCreatedNode(), Utils.toTerm(attr
					.getKey())));
	...
	private void visitAux(final ReturnVariableTerm term,
			final ImmutableList.Builder<Atom> builderAux) {
		if (Utils.isText(term)) {
			builderAux.add(new Atom(Predicate.TEXT.toString() + "_" + viewName,
					getParentBlockCreatedNode(term), term.getCreatedNode()));
		} else {
			...
			builderAux.add(new Atom(
					Predicate.CHILD.toString() + "_" + viewName,
					getParentBlockCreatedNode(term), term.getCreatedNode()));
			builderAux.add(new Atom(Predicate.TAG.toString() + "_" + viewName,
	...
	public void visit(final ReturnStringTerm term) {
		...
		if (isRootBlock()) {
			buildRootElement(builderAux,
					elementVariableFactory.createFreshVar());
			builderAux.add(new Atom(Predicate.TEXT.toString() + "_" + viewName,
					rootElementCreatedNode, term.toTerm()));
		} else {
			addParentBlockCreatedNode(builderAux, term);
			builderAux.add(new Atom(Predicate.TEXT.toString() + "_" + viewName,
	...
	private void addParentBlockCreatedNode(
		...
		if (auxiliarBuilders.containsKey(getParentBlockCreatedNode(term))) {
			...
			throw new IllegalStateException(
					"Parent block created node builder expected on child term.");
		}
	}
