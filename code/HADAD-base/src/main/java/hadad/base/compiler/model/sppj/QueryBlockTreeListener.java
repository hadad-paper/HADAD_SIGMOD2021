...
final class QueryBlockTreeListener {
    ...
    public QueryBlockTreeListener(final BlockListener blockListener,
            ...
            @Named("SPPJQLVariableFactory") final VariableFactory SPPJqlVariableFactory,
            final VariableMapper variableMapper) {
        this.blockListener = blockListener;
        this.cqVariableFactory = cqVariableFactory;
        this.aqlVariableFactory = SPPJqlVariableFactory;
     ...
     * 
     * @param str
     *            the SPSPPJQL query that needs to be parsed
     * @return
    ...
    public QueryBlockTree parse(final String str) throws ParseException {
        ...
        final ParserRuleContext tree = parser.sppjqlQuery();
        ...
        if (listener.hasRootElement()) {
            ...
            terms.add(parent);
            root = new RootBlock(listener.getQueryName(), new ReturnTemplate(SPPJModel.ID, terms));
    ...
    private class NestedBlockTreeListenerAux extends SPPJQLBaseListener {
        ...
        @Override
        public void enterSppjqlQuery(SPPJQLParser.SppjqlQueryContext ctx) {
            LOGGER.debug("Entering SPPJQLQuery: " + ctx.getText());
        ...
        @Override
        public void enterViewName(SPPJQLParser.ViewNameContext ctx) {
            LOGGER.debug("Entering ViewName: " + ctx.getText());
            queryName = ctx.getText();
        }

        @Override
        public void enterSppjRootFieldName(SPPJQLParser.SppjRootFieldNameContext ctx) {
            LOGGER.debug("Entering RootFieldName: " + ctx.getText());
