...
@Singleton
class PJBackwardEncoderNestedBlockTreeVisitor implements QueryBlockTreeVisitor {
	...
	public void visit(final RootBlock block) {
		if (!block.getPattern().isEmpty()) {
			final List<Constraint> skolemFunctions = Utils.getConstraintForSkolemFunctions(block,
	...
	private List<Constraint> getConstraintConnectingCreateToJSONConstructedByView(final RootBlock block) {
		final List<List<Atom>> premises = pJBlockBackwardEncoderReturnTermVisitor.encode(block.getReturnTemplate(),
				block.getQueryName());
	...
	private List<Constraint> getConstraintConnectingCreateToJSONConstructedByView(final ChildBlock block) {
		final List<List<Atom>> premises = pJBlockBackwardEncoderReturnTermVisitor.encode(block.getCreatedNode(),
