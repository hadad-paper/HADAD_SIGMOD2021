...
@Singleton
public class XQDocumentNamesExtractor implements QueryBlockTreeVisitor {
	private final String documentNamePrefix;
	private Set<String> documentNames;

	@Inject
	public XQDocumentNamesExtractor(@Named("document_name_prefix") final String documentNamePrefix) {
	...
	private void _visit(final Block block) {
		block.getPattern()
				.encoding(Utils.conditionEncoding)
				.stream()
				.filter(a -> a.getPredicate().startsWith(
						Predicate.ROOT.toString()))
				.map(a -> a.getPredicate().substring(
						a.getPredicate().lastIndexOf(documentNamePrefix) + 1))
