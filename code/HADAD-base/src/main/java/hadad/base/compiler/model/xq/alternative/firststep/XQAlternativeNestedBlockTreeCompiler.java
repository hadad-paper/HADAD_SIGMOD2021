 ...
 * 
 */
public final class XQAlternativeNestedBlockTreeCompiler implements QueryBlockTreeViewCompiler {
    ...
    private final XQDocumentNamesExtractor xQDocumentNamesExtractor;

    @Inject
    public XQAlternativeNestedBlockTreeCompiler(@Named("document_name_prefix") final String documentNamePrefix,
            ...
            final XQDocumentNamesExtractor xQDocumentNamesExtractor) {
        ...
        this.xQDocumentNamesExtractor = xQDocumentNamesExtractor;
    }

    @Override
    public List<? extends Constraint> compileForwardConstraints(final QueryBlockTree nbt, boolean includeComments)
            throws CompilationException {
        final Set<String> documentNames = xQDocumentNamesExtractor.getDocumentNames(nbt);
    ...
    private boolean isInTargetSchema(final String viewName, final Relation relation) throws CompilationException {
        if (!relation.getName().startsWith(Predicate.COPY.toString())
                && (relation.getName().equals(viewName) || relation.getName().endsWith(viewNamePrefix + viewName)
                        || relation.getName().endsWith(Predicate.CREATE.toString()))) {
            return true;
        }
        return false;
    }
