...
/**
 * AJ AJAlternativeNestedBlockTreeCompiler 
 */
public final class AJAlternativeNestedBlockTreeCompiler implements QueryBlockTreeViewCompiler {
    ...
    private final ForwardEncoderNestedBlockTreeVisitor forwardEncoderVisitor;
    private final BackwardEncoderNestedBlockTreeVisitor backwardEncoderVisitor;
    private final AJDocumentNamesExtractor aJDocumentNamesExtractor;

    @Inject
    public AJAlternativeNestedBlockTreeCompiler(@Named("document_name_prefix") final String documentNamePrefix,
            ...
            final ForwardEncoderNestedBlockTreeVisitor forwardEncoderVisitor,
            final BackwardEncoderNestedBlockTreeVisitor backwardEncoderVisitor,
            final AJDocumentNamesExtractor aJDocumentNamesExtractor) {
        ...
        this.aJDocumentNamesExtractor = aJDocumentNamesExtractor;
    }

    @Override
    public List<? extends Constraint> compileForwardConstraints(final QueryBlockTree nbt, boolean includeComments)
            throws CompilationException {
        final Set<String> documentNames = aJDocumentNamesExtractor.getDocumentNames(nbt);
    ...
    public RelationalSchema compileGlobalSchema(final QueryBlockTree nbt) throws CompilationException {
        ...
        final Relation tag = new Relation(Predicate.TAG.toString(), 2);
        if (!relations.contains(tag)) {
            relations.add(tag);
        }
    ...
    public RelationalSchema compileTargetSchema(final QueryBlockTree nbt) throws CompilationException {
        ...
        final Relation tag = new Relation(Predicate.TAG.toString() + viewNamePrefix + nbt.getQueryName(), 2);
        if (!targetSchemaRelations.contains(tag)) {
            targetSchemaRelations.add(tag);
        }
    ...
    private boolean isInTargetSchema(final String viewName, final Relation relation) throws CompilationException {
        if (!relation.getName().startsWith(Predicate.COPY.toString())
                && (relation.getName().equals(viewName) || relation.getName().endsWith(Predicate.CREATE.toString()))) {
            return true;
        }
        return false;
    }
